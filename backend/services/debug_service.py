...
class DebugService:
    ...
    def do_debug(self):
        ...
        X, y = feature_model.create_features(raw_data)
        ...
        X, _ = scaler.fit_transform(X, y, "buy_signal")
        ...
        y_pred = pd.DataFrame(y_pred, columns=["buy_signal"], index=X_test.index)
        ...
        y_pred = pd.DataFrame(y_pred, columns=["buy_signal"], index=X.index)
